...
def random_sample_rotation_z(rotation_factor: float = 1.0) -> np.ndarray:
    # angle_z, angle_y, angle_x
    # (0, 2 * pi / rotation_range)
    euler_z = np.random.rand(1) * np.pi * 2 / rotation_factor
    euler_yx = (np.random.rand(2) - 0.5) * np.pi * 2 / (rotation_factor * 20)
    rotation = Rotation.from_euler('zyx', [euler_z, euler_yx[0], euler_yx[1]]).as_matrix()
    return rotation
...
def random_sample_transform(rotation_magnitude: float, translation_magnitude: float) -> np.ndarray:
    euler = np.random.rand(3) * np.pi * \
        rotation_magnitude / 180.0  # (0, rot_mag)
    rotation = Rotation.from_euler('zyx', euler).as_matrix()
    translation = np.random.uniform(-translation_magnitude,
                                    translation_magnitude, 3)
...
def random_sample_keypoints_with_scores(
    ...
    if num_points > num_keypoints:
        ...
        indices = np.random.choice(
            indices, num_keypoints, replace=False, p=probs)
        points = points[indices]
        feats = feats[indices]
    return points, feats
        ...
        for i in range(num_points):
            if masks[i]:
                ...
                if len(indices) == num_keypoints:
                    break
                if i + 1 < num_points:
                    current_masks = np.sum(
                        (sorted_points[i + 1:] - sorted_points[i]) ** 2, axis=1) < radius2
