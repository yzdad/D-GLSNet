...
class GroupNorm(nn.Module):
    ...
    def forward(self, x):
        x = x.transpose(0, 1).contiguous().unsqueeze(0)  # (N, C) -> (B, C, N)
        x = self.norm(x)
        x = x.squeeze(0).transpose(0, 1).contiguous()  # (B, C, N) -> (N, C)
        return x.squeeze()


class UnaryBlock(nn.Module):
    def __init__(self, in_channels, out_channels, group_norm, has_relu=True, bias=False, layer_norm=False):
...
class ResidualBlock(nn.Module):
    def __init__(
        ...
        self.KPConv = KPConv(mid_channels, mid_channels, kernel_size, radius, sigma, bias=bias)  # [N, C]
        if layer_norm:
