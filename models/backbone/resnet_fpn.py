...
class ResNetFPN_8_2(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Networks
        self.conv1 = nn.Conv2d(1, initial_dim, kernel_size=7,
                               stride=2, padding=3, bias=False)  # 1/2
        self.bn1 = nn.Identity()
        self.relu = nn.ReLU(inplace=True)

        # 1/2  initial_dim  ->block_dims[0]
        self.layer1 = self._make_layer(block, block_dims[0], stride=1)
        # 1/4  block_dims[0]->block_dims[1]
        self.layer2 = self._make_layer(block, block_dims[1], stride=2)
        # 1/8  block_dims[1]->block_dims[2]
...
class ResNetFPN_16_4(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Networks
        self.conv1 = nn.Conv2d(1, initial_dim, kernel_size=7,  stride=2, padding=3, bias=False)  # 1/2
        self.bn1 = nn.GroupNorm(32, initial_dim)
        self.relu = nn.LeakyReLU(0.1, inplace=True)
